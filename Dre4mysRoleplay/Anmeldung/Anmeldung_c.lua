...
function blackBalken()
    ...
    dxDrawRectangle(5*(x/1440), 851*(y/900), 1440*(x/1440), 49*(y/900), tocolor(0, 0, 0, 255), false)
end
...
addEventHandler('onClientResourceStart', resourceRoot,function()
	if(not(isElement(Ameldung.window[1])))then
		...
		fadeCamera(true)
		showCursor(true)
		music = playSound('Sounds/Anmeldebereich.mp3')
		...
		addEventHandler('onClientGUIClick',Ameldung.button[1],function()
			...
			if(passwort == '')then
				...
				if(passwort:len() >= 4)then
					triggerServerEvent('Dre4mYEinloggen',localPlayer,name,passwort)
				else
					if(german == true)then
		...
		addEventHandler('onClientGUIClick',Ameldung.button[2],function()
			...
			if(passwort == '')then
				...
				if(passwort:len() >= 4)then
					if(german == true)then
						triggerServerEvent('Dre4mYRegistrieren',localPlayer,name,passwort,'0')
					elseif(english == true)then
						triggerServerEvent('Dre4mYRegistrieren',localPlayer,name,passwort,'1')
					end
				else
					if(german == true)then
