...
function motelWindow()
	if(getElementData(localPlayer,'Dre4mYClick') == false)then
		if(not(isElement(motel.window[1])))then
			...
			motel.label[1] = guiCreateLabel(0.51, 0.17, 0.46, 0.77, 'Willkommen im Fort Carson Motel. Für 630$ pro Payday kannst du dir hier ein Zimmer mieten.', true, motel.window[1])
			...
			if(getElementData(localPlayer,'Language') == 1)then
				...
				guiSetText(motel.label[1],'Welcome to the Fort Carson Motel. For $ 630 per Payday can you rent a room here.')
			end
